...
export const Admin = () => {
    ...
    const tickets = signups.filter((s) => !s.intent || s.intent === "ticket");
    const waitlist = signups.filter((s) => s.intent === "waitlist");
    const filteredTickets = ticketFilter === "all" ? tickets : tickets.filter((s) => s.tier === ticketFilter);
    const tierCount = (t: string) => tickets.filter((s) => s.tier === t).length;
                ...
                <div className="flex gap-px bg-[#f0ede6]/10 mb-10 w-fit">
                    {(["tickets", "waitlist"] as Tab[]).map((t) => (
                        <button
                            key={t}
                            onClick={() => setTab(t)}
                            className={`px-8 py-3 text-[10px] tracking-[0.2em] uppercase transition-all duration-300 ${tab === t ? "bg-[#c9a96e] text-[#0e0e0e]" : "bg-[#0e0e0e] text-[#f0ede6]/50 hover:text-[#f0ede6]"
                                }`}
                        >
                            {t === "tickets" ? `Tickets (${tickets.length})` : `Waitlist (${waitlist.length})`}
                        </button>
                    ))}
                </div>
                    ...
                    <div>
                        ...
                        <div className="flex gap-2 flex-wrap mb-6">
                            {["all", "general", "vip", "exhibitor", "sponsor"].map((t) => (
                                <button key={t} onClick={() => setTicketFilter(t)}
                                    className={`px-4 py-2 text-[10px] tracking-[0.15em] uppercase transition-colors ${ticketFilter === t ? "bg-[#c9a96e] text-[#0e0e0e]" : "border border-[#f0ede6]/15 text-[#f0ede6]/60 hover:border-[#f0ede6]/40"
                                        }`}>
                                    {t}
                                </button>
                            ))}
